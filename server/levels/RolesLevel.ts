...
export class RolesLevel extends BaseLevel {
  ...
  private disableDeadline(): void {
    ...
    this.removeDeadlineTimeout = setTimeout(() => {
      ...
      if (!this.state.rolesLevel.complete && !this.state.isGameOver) {
  ...
  private resetState(): void {
    ...
    rs.machines.clear();
    rs.processedCount = 0;
    rs.itemState = "RAW_STOCK";
    rs.statusMessage = "Move the raw stock through Machine 1, then continue in numerical order.";
    rs.complete = false;
    this.state.stage = 1;
  ...
  private generateFactoryLayout(phaseIndex: number): void {
    ...
    belts.forEach((belt) => rs.conveyors.push(belt));
    ...
    rs.itemX = itemPoint.x;
    rs.itemY = itemPoint.y;
    this.syncPlayersToItem();
  }

  canPlayerMoveTo(player: Player, x: number, y: number): boolean {
    if (this.state.rolesLevel.complete || this.state.isGameOver) return false;
    ...
    this.pendingMoves.set(player.sessionId, {
      from: { x: this.state.rolesLevel.itemX, y: this.state.rolesLevel.itemY },
      to: move.destination,
      beltId: move.beltId,
    });
    return true;
  ...
  onPlayerMove(player: Player, _x: number, _y: number): void {
    ...
    if (!rs.complete && !hitWrongMachine) {
      rs.statusMessage = `Part moved by ${this.roleLabel(player.role as PlayerRole)}. Next stop: Machine ${rs.processedCount + 1}.`;
    }
  }

  private findDestination(role: PlayerRole, direction: Direction): { destination: Point; beltId: string } | null {
    const rs = this.state.rolesLevel;
    const item = { x: rs.itemX, y: rs.itemY };
    const candidates: Array<{ destination: Point; beltId: string }> = [];

    for (const belt of rs.conveyors) {
  ...
  private processMachinesOnPath(from: Point, to: Point): boolean {
    const rs = this.state.rolesLevel;
    const machinesOnPath = Array.from(rs.machines)
    ...
    for (const machine of machinesOnPath) {
      if (machine.order <= rs.processedCount) continue;
      if (machine.order !== rs.processedCount + 1) {
        rs.statusMessage = `Wrong station. Send the part through Machine ${rs.processedCount + 1} before Machine ${machine.order}.`;
        return true;
      }

      rs.processedCount = machine.order;
      rs.lights = machine.order % MACHINES_PER_PHASE || MACHINES_PER_PHASE;
      rs.itemState = machine.order >= TOTAL_MACHINE_COUNT ? "WIRE_CUTTER" : `PROCESSED_${machine.order}`;

      if (machine.order >= TOTAL_MACHINE_COUNT) {
        rs.complete = true;
        rs.statusMessage = "Wire cutter complete. Factory run successful.";
  ...
  private expandFactoryFloor(): void {
    const rs = this.state.rolesLevel;
    const phaseIndex = Math.min(Math.floor(rs.processedCount / MACHINES_PER_PHASE), PHASES.length - 1);
    this.generateFactoryLayout(phaseIndex);
    rs.stage = phaseIndex + 1;
    this.state.stage = phaseIndex + 1;
    rs.lights = 0;
    rs.statusMessage = `Factory floor expanded. Machine ${rs.processedCount + 1} is online.`;
  }
  ...
  private addMachine(order: number, point: Point): void {
    ...
    machine.x = point.x;
    machine.y = point.y;
    this.state.rolesLevel.machines.push(machine);
  }

  private syncPlayersToItem(): void {
    const { itemX, itemY } = this.state.rolesLevel;
    this.state.players.forEach((player) => {
      player.x = itemX;
      player.y = itemY;
    });
  ...
  private insideBounds(point: Point, bounds: ReturnType<RolesLevel["bounds"]>): boolean {
  ...
  isLevelComplete(): boolean {
    return this.state.rolesLevel.complete;
  }
