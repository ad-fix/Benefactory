...
interface FactoryLevelContextValue {
  rolesLevel: FactoryState;
  gridWidth: number;
  gridHeight: number;
...
const FactoryLevelContext = createContext<FactoryLevelContextValue | null>(null);

export const FactoryLevelProvider = ({
  children,
  ...value
}: FactoryLevelContextValue & { children: ReactNode }) => (
  <FactoryLevelContext.Provider value={value}>{children}</FactoryLevelContext.Provider>
);
...
export const RolesLevelView = ({ role }: RolesLevelViewProps) => {
  const factory = useContext(FactoryLevelContext);
  if (!factory) return null;
  if (factory.rolesLevel.conveyors.length === 0) {
    return <FactoryWaitingRoom playersConnected={factory.playersConnected} roomId={factory.roomId} />;
  }
  return <FactoryBoard role={role} {...factory} />;
};

const FactoryWaitingRoom = ({
  playersConnected,
  roomId,
}: Pick<FactoryLevelContextValue, "playersConnected" | "roomId">) => (
...
const FactoryBoard = ({
  ...
  gridWidth,
  gridHeight,
}: RolesLevelViewProps & FactoryLevelContextValue) => {
  ...
  const beltsAtItem = useMemo(
    () => new Set(rolesLevel.conveyors.filter((belt) => beltContainsPoint(belt, rolesLevel.itemX, rolesLevel.itemY)).map((belt) => belt.id)),
    [rolesLevel.conveyors, rolesLevel.itemX, rolesLevel.itemY],
  );

  const sortedMachines = useMemo(
    () => [...rolesLevel.machines].sort((a, b) => a.order - b.order),
    [rolesLevel.machines],
  );
    ...
    <div className="pointer-events-none absolute inset-0 z-[30] font-mono text-[#ebe5cf]">
      ...
      >
        ...
        {sortedMachines.map((machine) => {
          const active = machine.order === rolesLevel.processedCount + 1;
          const completed = machine.order <= rolesLevel.processedCount;
          return (
            <div
              key={machine.id}
        ...
        <div
          ...
          style={position(rolesLevel.itemX, rolesLevel.itemY)}
        >
          {rolesLevel.complete ? <Scissors className="size-6" /> : <Package className="size-6" />}
        </div>
      </div>
      ...
      <section className="absolute right-[9%] top-4 flex max-w-[44vw] items-center gap-2 border border-[#ebe5cf]/20 bg-[#213242]/95 px-3 py-2 shadow-xl lg:gap-4 lg:px-4">
        ...
        <div>
          ...
          <p className="text-sm font-black uppercase tracking-[.12em]">Phase {rolesLevel.stage} / 3</p>
        </div>
